...
function generateQuestion() {
  ...
  else { a = Math.floor(Math.random() * 10) + 3; b = Math.floor(Math.random() * 10) + 3; answer = a * b; }
...
function generateChallengeCode() {
  ...
  let code;
  do { code = ""; for (let i = 0; i < 5; i++) code += chars[Math.floor(Math.random() * chars.length)]; } while (challengeCodes[code]);
  return code;
}
...
function startGame(p1, p2, timedMode) {
  ...
  const game = {
    gameId, isPractice: false, roundLocked: false, timedMode: !!timedMode,
    spectators: new Set(),
    players: {
    ...
    question, round: 1, maxScore: 5,
  };
  activeGames[gameId] = game;
  p1.socket.join(gameId); p2.socket.join(gameId);
  p1.socket.gameId = gameId; p2.socket.gameId = gameId;
  io.to(gameId).emit("game:start", {
    gameId, isPractice: false, timedMode: game.timedMode,
    opponent: { [p1.socket.id]: p2.username, [p2.socket.id]: p1.username },
    question: question.question, round: 1,
...
function queueRoundTimeout(gameId) {
  ...
  setTimeout(() => {
    const current = activeGames[gameId]; if (!current) return;
    if (current.round !== roundAtSchedule) return; if (current.roundLocked) return;
    current.roundLocked = true;
    io.to(gameId).emit("game:timeout", { question: current.question.question, answer: current.question.answer });
...
async function endGame(gameId, winnerSocketId) {
  const game = activeGames[gameId]; if (!game) return;
  ...
  if (game.isPractice) {
    ...
    io.to(gameId).emit("game:over", { isPractice: true, winner: winner.username, eloChange: 0, players: {} });
    await recordMatch(human.username, winner.username, winner.username, 0, true);
  ...
  const isFarming = winnerSocket?.deviceId && loserSocket?.deviceId && winnerSocket.deviceId === loserSocket.deviceId;
  const eloChange = isFarming ? 0 : baseEloChange;
  if (isFarming) console.log(`ELO farming detected: ${winner.username} vs ${loser.username}`);

  await updateElo(winner.username, +eloChange, true);
  await updateElo(loser.username, -eloChange, false);

  const winnerFinal = await getPlayer(winner.username);
  const loserFinal = await getPlayer(loser.username);

  io.to(gameId).emit("game:over", {
    isPractice: false, winner: winner.username, eloChange,
    players: { [winner.username]: { ...winnerFinal }, [loser.username]: { ...loserFinal } },
  });

  await recordMatch(winner.username, loser.username, winner.username, eloChange, false);
  delete activeGames[gameId];
...
function startPracticeGame(socket, difficulty) {
  ...
  const game = {
    gameId, isPractice: true, roundLocked: false, timedMode: false,
    cpuName, cpuSocketId: cpuId, cpuDiff: diff,
    players: {
  ...
  socket.emit("game:start", {
    gameId, isPractice: true, timedMode: false,
...
function queueCpuMove(gameId) {
  const game = activeGames[gameId]; if (!game || !game.isPractice) return;
  const { minDelay, maxDelay, accuracy } = game.cpuDiff;
  const delay = minDelay + Math.random() * (maxDelay - minDelay);
  const roundAtSchedule = game.round;
  setTimeout(() => {
    const current = activeGames[gameId]; if (!current) return;
    if (current.round !== roundAtSchedule) return; if (current.roundLocked) return;
    const cpu = current.players[current.cpuSocketId]; if (!cpu || cpu.answered) return;
    const willBeCorrect = Math.random() < accuracy; cpu.answered = true;
    if (willBeCorrect) {
      current.roundLocked = true; cpu.score++; cpu.streak = (cpu.streak || 0) + 1;
      const humanId = Object.keys(current.players).find((id) => id !== current.cpuSocketId);
      current.players[humanId].streak = 0;
      const scores = Object.fromEntries(Object.values(current.players).map((p) => [p.username, p.score]));
      io.to(gameId).emit("game:point", { scorer: cpu.username, scores, streak: cpu.streak });
      if (cpu.score >= current.maxScore) { endGame(gameId, current.cpuSocketId); }
...
io.on("connection", (socket) => {
  ...
  socket.on("auth:signup", async ({ username, password, avatar, deviceId }) => {
    const trimmed = (username || "").trim();
    const pass = (password || "").trim();
    if (trimmed.length < 2) return socket.emit("auth:error", { message: "Username must be at least 2 characters!" });
    if (pass.length < 4) return socket.emit("auth:error", { message: "Password must be at least 4 characters!" });
    if (await usernameExists(trimmed)) return socket.emit("auth:error", { message: "That username is already taken!" });
    const player = await createPlayer(trimmed, pass, avatar);
    socket.username = player.username; socket.elo = player.elo; socket.deviceId = deviceId;
    if (deviceId) { if (!deviceAccounts[deviceId]) deviceAccounts[deviceId] = new Set(); deviceAccounts[deviceId].add(trimmed); }
    socket.emit("auth:success", player);
    console.log(`New account created: ${trimmed}`);
  });

  socket.on("auth:login", async ({ username, password, deviceId }) => {
    const trimmed = (username || "").trim();
    const pass = (password || "").trim();
    if (!await usernameExists(trimmed)) return socket.emit("auth:error", { message: "No account with that username. Sign up instead!" });
    if (!await verifyPassword(trimmed, pass)) return socket.emit("auth:error", { message: "Wrong password!" });
    const player = await getPlayer(trimmed);
    socket.username = player.username; socket.elo = player.elo; socket.deviceId = deviceId;
    if (deviceId) { if (!deviceAccounts[deviceId]) deviceAccounts[deviceId] = new Set(); deviceAccounts[deviceId].add(trimmed); socket.altAccountFlag = deviceAccounts[deviceId].size > 1; }
    socket.emit("auth:success", player);
    console.log(`${trimmed} logged in${socket.altAccountFlag ? " [ALT]" : ""}`);
  });

  socket.on("player:leaderboard", async () => {
    socket.emit("leaderboard:data", await getLeaderboard());
  });
...
socket.on("matchmaking:join", () => {
  ...
  socket.on("challenge:create", () => {
    ...
    console.log(`${socket.username} created challenge ${code}`);
  });

  socket.on("challenge:cancel", () => {
    if (socket.challengeCode && challengeCodes[socket.challengeCode]) {
      delete challengeCodes[socket.challengeCode]; socket.challengeCode = null;
    }
  });
  ...
  socket.on("challenge:join", ({ code }) => {
    ...
    if (!challenge) return socket.emit("challenge:error", { message: "Invalid or expired code!" });
    if (challenge.socket.id === socket.id) return socket.emit("challenge:error", { message: "You can't challenge yourself!" });
    delete challengeCodes[trimmedCode];
    startGame({ socket: challenge.socket, username: challenge.username, elo: challenge.elo }, { socket, username: socket.username, elo: socket.elo }, true);
  });

  socket.on("spectate:list", async () => {
    const liveGames = Object.values(activeGames).filter((g) => !g.isPractice).map((g) => {
      const [p1, p2] = Object.values(g.players);
      return { gameId: g.gameId, player1: p1.username, player2: p2.username, score1: p1.score, score2: p2.score, round: g.round };
    });
  ...
  socket.on("spectate:join", ({ gameId }) => {
    if (!socket.username) return;
    const game = activeGames[gameId];
    if (!game || game.isPractice) return socket.emit("spectate:error", { message: "That match has ended." });
    if (game.players[socket.id]) return socket.emit("spectate:error", { message: "You can't spectate your own match!" });
    game.spectators.add(socket.id); socket.spectatingGameId = gameId; socket.join(gameId);
    const [p1, p2] = Object.values(game.players);
    socket.emit("spectate:joined", { gameId, player1: p1.username, player2: p2.username, score1: p1.score, score2: p2.score, question: game.question.question, round: game.round });
    console.log(`${socket.username} spectating ${gameId}`);
  });

  socket.on("spectate:leave", () => {
    const gameId = socket.spectatingGameId;
    if (gameId && activeGames[gameId]) { activeGames[gameId].spectators.delete(socket.id); socket.leave(gameId); }
    socket.spectatingGameId = null;
  });

  socket.on("game:answer", ({ answer }) => {
    const gameId = socket.gameId; if (!gameId) return;
    const game = activeGames[gameId];
    if (!game) { console.log(`Answer after game ended`); return; }
    if (game.roundLocked) return;
    const player = game.players[socket.id];
    if (!player || player.answered) { console.log(`Duplicate answer from ${socket.username}`); return; }
    ...
    if (correct) {
      game.roundLocked = true; player.score++; player.streak = (player.streak || 0) + 1;
  ...
  socket.on("disconnect", async () => {
    const idx = matchmakingQueue.findIndex((p) => p.socket.id === socket.id);
    if (idx !== -1) matchmakingQueue.splice(idx, 1);
    if (socket.challengeCode && challengeCodes[socket.challengeCode]) delete challengeCodes[socket.challengeCode];
    if (socket.spectatingGameId && activeGames[socket.spectatingGameId]) activeGames[socket.spectatingGameId].spectators.delete(socket.id);
    ...
    if (gameId && activeGames[gameId]) {
      ...
      if (!game.isPractice) {
        ...
        if (remainingId) {
          const winner = game.players[remainingId]; const loser = game.players[socket.id];
...
server.listen(PORT, () => { console.log(`Math Duel server running at http://localhost:${PORT}`); });